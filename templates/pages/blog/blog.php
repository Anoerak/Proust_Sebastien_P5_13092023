...
<section class="project__container">
	...
	<div class="main__container">
		...
		if (isset($_SESSION['logged_user']) && ($_SESSION['privileges']['privilege'] === 'admin')) : ?>
			<a href="index.php?page=post&action=get&option=Add" class="add__post__button main__button">
				Add a post
			</a>
		<?php endif; ?>
		...
		<article>
			<?php foreach ($datas as $post) : ?>
				<div class="card <?= $post->id ?>" style="background-image: url('<?= $post->picture ?>');">
					<a href="index.php?page=post&action=getOne&option=view&id=<?= $post->id ?>" class="projects__details">More
						Informations</a>
