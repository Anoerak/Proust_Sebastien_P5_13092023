			...
			<div id="<?= $comment->id ?>" class="comment__container">
				...
				<?php if (isset($_SESSION['logged_user']) && $_SESSION['user_id'] == $comment->user_id && $comment->validation_status === 'published' && $comment->id == $_REQUEST['comment_id']) : ?>
					<form action="index.php?page=post&action=update&option=comment&id=<?= $comment->id ?>&post_id=<?= $_GET['id'] ?>" method="post" class="edited__comment">
						...
						<textarea cols="60" rows="7" type="text" class="comment__content" name="comment" id="comment"><?= $comment->content ?></textarea>
				...
				<div id="<?= $comment->id ?>" class="comment__validation">
					...
					?>"><b>Status : </b><em><?= $comment->validation_status ?></em></p>
						...
						<?php if (isset($_SESSION['logged_user']) && $_SESSION['user_id'] == $comment->user_id && $comment->validation_status === 'published') : ?>
							<button class="main__button__light" onclick="window.location.href='index.php?page=post&action=get&option=view&id=<?= $_GET['id'] ?>&comment_id=<?= $comment->id ?>#<?= $comment->id ?>'">Edit</button>
						<?php endif; ?>
					</div>
					<!-- If you're an admin, we display buttons to validate or remove comments -->
					<?php if (isset($_SESSION['logged_user']) && $_SESSION['privileges']['privilege'] === 'admin') : ?>
						<div class="comment__actions">
							<button class="main__button" onclick="window.location.href='index.php?page=post&action=validate&option=comment&id=<?= $comment->id ?>&post_id=<?= $_GET['id'] ?>'" <?php if ($comment->validation_status === 'published') : ?>disabled<?php endif; ?>>Publish</button>
							<button class="alert__button" onclick="window.location.href='index.php?page=post&action=refuse&option=comment&id=<?= $comment->id ?>&post_id=<?= $_GET['id'] ?>'" <?php if ($comment->validation_status === 'unpublished') : ?>disabled<?php endif; ?>>Unpublish</button>
